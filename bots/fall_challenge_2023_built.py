...
def save_points(my_drones: Dict[int, MyDrone], owners_scores: Dict[int, int], owners_max_possible_score: Dict[int, int], owners_extra_score_with_all_unsaved_creatures: Dict[int, int], my_owner=MY_OWNER, foe_owner=FOE_OWNER):
    actions = {}
    extra_score_to_win = owners_max_possible_score[foe_owner] - owners_scores[my_owner] + 1
    ...
    if extra_score_if_all_my_drones_save >= extra_score_to_win:
        for drone in my_drones.values():
            if len(drone.unsaved_creatures_idt) > 0:
                actions[drone.idt] = Action(target=Point(drone.x, 499), comment=f'SAVE {extra_score_if_all_my_drones_save} ({extra_score_to_win})')
    else:
        ...
        for drone in ordered_my_drones_with_most_extra_score:
            ...
            if drone_extra_score >= 20 or drone_extra_score >= extra_score_to_win:
                actions[drone.idt] = Action(target=Point(drone.x, 499), comment=f'SAVE {drone.extra_score_with_unsaved_creatures} ({extra_score_to_win})')
    return actions
...
def just_do_something(my_drones: Dict[int, MyDrone], creatures: Dict[int, Creature]):
    ...
    for drone_idt, drone in my_drones.items():
        if drone.extra_score_with_unsaved_creatures > 0:
            actions[drone.idt] = Action(target=Point(drone.x, 499), comment=f'SAVE {drone.extra_score_with_unsaved_creatures}')
        else:
...
def flee_from_monsters(my_drones: Dict[int, MyDrone], monsters: List[Creature], nb_turns: int, hash_map_norms=HASH_MAP_NORMS, flee_radius_from_monsters=FLEE_RADIUS_FROM_MONSTERS, safe_radius_from_monsters=SAFE_RADIUS_FROM_MONSTERS, map_center=MAP_CENTER, drone_speed=DRONE_MAX_SPEED):
    actions = {}
    ...
    for drone_idt, drone in my_drones.items():
        ...
        if len(drone_has_to_flee_from) == 1:
            ...
            if distance_to_creature > safe_radius_from_monsters:
                ...
                actions[drone_idt] = Action(target=drone.position + drone_speed ** (1 / 2) * flee_vector, comment=f'FLEE FROM {monster.log()}')
            else:
                flee_vector = -1 * vector_to_creature
                actions[drone_idt] = Action(target=drone.position + drone_speed ** (1 / 2) / distance_to_creature ** (1 / 2) * flee_vector, comment=f'FLEE FROM {monster.log()}')
...
def evaluate_extra_scores_for_multiple_scenarios(creatures: Dict[int, Creature], my_drones: Dict[int, MyDrone], foe_drones: Dict[int, FoeDrone], scans: Dict[int, Scans], trophies: Trophies, current_owners_scores: Dict[int, int], my_owner=MY_OWNER, foe_owner=FOE_OWNER, owners=OWNERS, monster_kind=Kind.MONSTER.value):
...
class GameLoop:
    __slots__ = ('init_inputs', 'nb_turns', 'turns_inputs', 'game_assets', 'empty_array_saved_creatures', 'max_number_of_radar_blips_used', 'max_speed_per_kind', 'corners', 'my_owner', 'foe_owner', 'owners', 'owners_scores', 'owners_extra_score_with_all_unsaved_creatures', 'owners_max_possible_score', 'my_drones_idt_play_order', 'newly_saved_creatures', 'monsters')
    RUNNING = True
    LOG = True
    ...
    def start(self):
        while GameLoop.RUNNING:
            ...
            for drone_idt in self.my_drones_idt_play_order:
                ...
                safe_action = avoid_monsters_while_aiming_for_an_action(drone=my_drones[drone_idt], aimed_action=aimed_action, monsters=self.monsters, nb_turns=self.nb_turns)
